...
def compress(content):
    return content
    compressed = zstd.compress(content.encode('utf-8'), -2)
    res = base64.b64encode(compressed)
    res2 = res.decode('ascii')
    logger.info(f'{len(compressed)} vs {len(content)} vs {len(res2)}')
    return res2
...
class PublicationSlot:
    def __init__(self, publisher, status, blockchainAddress = None, updateCount = 0, readerEndpoint = None, readerUrl = None):
        ...
        self.receiver_url = publisher
        self.retentionDate = Utils.getRandomRetention()
        self.batching_level = 1
...
class SinglePublisherState:
    ...
    def addToReport(
        ...
        with self.mut.publisherLock:
            ...
            if success:
                self.result.publishedOk += batching_level
                ...
                self.result.publishedFail += batching_level
    ...
    def getNextPdf(self, sufix=""):
        with mutex:
            ...
            cif = None
            receiver_url = self.url
            title = 'RandomText_i' + sufix

            if not self.conf.use_predefined_pdfs:
                content = self.getRandomContent(int(self.conf.sizeKB), sufix)
                return [content], additional_details, documentMainCategory, [title], cif, receiver_url, 1

            if self.gen is None:
                self.gen = self.conf.pdf_getter(self.url)
            try:
                from pdfrw import PdfReader, PdfWriter
                pdf_path, additional_details, documentMainCategory, title, cif, receiver_url, batching_level = next(self.gen)
            except Exception as err:
                print(err)
                raise err
                return None, additional_details,  documentMainCategory, title, cif, receiver_url, 1
            result_pdfs = []
            for pdf in pdf_path:
                ...
                result_pdfs.append(content)
        return result_pdfs, additional_details, documentMainCategory, title, cif, receiver_url, batching_level
    ...
    def calculateQueue(self, minimum, maximum):
        ...
        if maximum < 0:
            ...
            elif 9 * self.mut.localPublishedFail >= self.mut.localPublishedOk:
                if int(max_active_now / 1.5 < minimum):
                    max_active_now = minimum
                else:
                    max_active_now = int(max_active_now / 1.5)
            elif 19 * self.mut.localPublishedFail >= self.mut.localPublishedOk:
                pass
            else:
        ...
        try:
            ...
            while repeatPub and not global_state.exit.is_set():
                ...
                if pub.status == 'READY_TO_PUBLISH':
                    ...
                    if not pub.content:
                        pub.content, pub.additional_details, pub.documentMainCategory, titles, pub.cif, pub.receiver_url, pub.batching_level = self.getNextPdf(str(index) + 'A' + str(threadNo) + 'time' + str(start) + '@' + self.url)
                        pub.title = copy.copy(titles) # TODO reszta podobnie powinna dzialac
                        if len(pub.content) > 1:
                            pub.updateCount = len(pub.content) - 1
                            pub.content.reverse()
                            pub.title.reverse()
                    if pub.content is None or pub.title is None:
                        raise BaseException(f'Problem with pdf getter {pub.content is None} or {pub.title is None}')
                    if len(pub.content) != len(pub.title):
                        raise BaseException(f'Problem with pdf getter, {len(pub.content)} != {len(pub.title)}')
                    ...
                    title = pub.title.pop() or randomText
                    content = pub.content.pop()
                    try:
                        sendStartTime = self.conf.getTime()
                        if self.conf.private_for_publisher:
                            pub.cif = pub.cif or next(self.cif)
                            retPublish = self.getEndpoint().PublishPrivateDocument({
                                ...
                                'documentData': {
                                    ...
                                    'BLOCKCHAINexpirationDate': pub.retentionDate,
                                    'BLOCKCHAINretentionDate': pub.retentionDate,
                                    'extension': 'PDF',
                                    'additionalDetails': compress(pub.additional_details),
                            ...
                            retPublish = self.getEndpoint().PublishPrivateDocument({
                                'publisherCif': pub.cif or self.cif,
                                'publicationMode': 'NEW',
                                'documentData': {
                                    'title': title,
                                    'sourceDocument': content,
                    ...
                    try:
                        ...
                        if retRead.status.status == 'PUBLISHING-OK':
                            ...
                            self.addToReport(
                                ...
                                batching_level=pub.batching_level)
                            self.handleReadySlot(pub, loop_stats)
                            repeatPub = True
                            continue
        ...
        while self.mut.active > 0 and not global_state.exit.is_set():
            ...
            if self.mut.localPublishedOk + self.mut.localPublishedFail >= 100:
                ...
                if self.mut.max_active > old_max_active:
                    logger.info(f'Increasing concurrent publications for {self.url} from {old_max_active} to {self.mut.max_active}')
                    ...
                    logger.info(f'Decreasing concurrent publications for {self.url} from {old_max_active} to {self.mut.max_active}')
...
class DocsPublishingManager:
    def __init__(self, conf):
        ...
        for ip, ports in self.conf.pubs.items():
            for port in ports:
                soap_address = 'http://' + ip + ':' + str(port)
                self.publishers[soap_address] = self.conf.documents_to_publish_for_pub.get(ip + ':' + str(port), self.conf.documents_to_publish)
        self.MAX_WORKERS = max(len(self.publishers), 1)

        self.testDateStart = 0
        self.testDateEnd = 0
        self.testTime = 0
        self.timeoutTriggered = False
        self.reportName = "report_" + str(time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime())) + str(os.getpid()) +'.csv'
        self.publishedOk = 0
        self.publishedFail = 0
    ...
    def writeReportStart(self, name):
        with open(name, 'a') as report:
            if not self.writtenHeader:
                header = "doc_hash,md5,pub_task_id,pub_address,cif,dur_time,status,pub_end_time,threads,start_brg_time,create_brg_time,published_brg_time,dur_brg_time,time_to_init,read_time,prev_doc_hash,published_by,read_by, batching_level, host\n"
                report.write(header)
            if self.conf.write_on_disk:
    ...
    def writeReport(self, name, publications):
        ...
        with open(name, 'a') as report:
            for record in publications:
                record['host'] = host
                rec = "{doc_hash},{md5},{pub_task_id},{pub_address},{cif},{dur_time:.3f},{status:<11},{pub_end_time},{threads},{start_brg_time:.3f},{create_brg_time:.3f},{published_brg_time:.3f},{dur_brg_time:.3f},{time_to_init:.3f},{dur_read_time},{prev_doc},{published_by},{read_by}, {batching_level}, {host}\n".format_map(
                    record)
                report.write(rec)
    ...
    def doCategories(self, url):
        if url not in self.conf.pubs_categories.keys():
            self.conf.pubs_categories[url] = {'ROOT'}
        pubPort = url.split(":")[-1]
        pubIp = url.split(":")[1].strip('/')
        pubEndpoint = SoapAPI.PublisherEndpoint(pubIp, pubPort)
        try:
        ...
        if set(self.conf.pubs_categories[url]).issubset(set([cat.name for cat in categoriesInit])):
            logger.info(f'Categories {self.conf.pubs_categories[url]} already added to publicator {url}')
            print(f'Categories {self.conf.pubs_categories[url]} already added to {url}')
            return True

        categoriesInitLst = [{'name': cat.name, 'active': cat.active, 'parentPath': cat.parentPath} for cat in categoriesInit]
        categoriesToSet = [*categoriesInitLst, *[{'name': cat, 'active': True} for cat in self.conf.pubs_categories[url]]]
...
class DurableMediaTester:
    def __init__(self, params):
        ...
        parser.add_argument('--old_wsdl', action=argparse.BooleanOptionalAction)
...
def setupLogger(log_level, verbose):
    ...
    file_handler = logging.FileHandler(filename=f'DurableMediaTestLog_{os.getpid()}_{time_now_str}')
...
if __name__ == "__main__":
    ok = main(sys.argv[1:])
    logger.info(f' Script run with parameteres: {sys.argv[1:]}')
